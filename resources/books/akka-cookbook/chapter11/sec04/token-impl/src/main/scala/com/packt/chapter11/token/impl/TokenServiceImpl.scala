...
class TokenServiceImpl(implicit val ec: ExecutionContext) extends TokenService {
  ...
  override def retrieveToken: ServiceCall[RetrieveTokenRequest, RetrieveTokenResult] = ServiceCall { request =>
    Future {
  ...
  override def validateToken: ServiceCall[ValidateTokenRequest, ValidateTokenResult] = ServiceCall { request =>
    Future {
