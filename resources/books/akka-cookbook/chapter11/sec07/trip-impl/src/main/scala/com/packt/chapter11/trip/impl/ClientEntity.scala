...
class ClientEntity extends PersistentEntity {
  ...
  override def behavior: Behavior =
    Actions()
      ...
      case (_, ctx, state) if state.tripInProgress =>
        ctx.thenPersist(TripEnded(new Date().getTime)) { _ =>
          ctx.reply(Done)
        }
      case (_, ctx, _) =>
      ...
      case (AddLocation(req), ctx, state) if
      state.tripInProgress =>
        ctx.thenPersist(LocationAdded(Location(req.latitude,
          req.longitude))) { _ => ctx.reply(Done) }
      case (_, ctx, _) =>
        ctx.invalidCommand("The trip has not started.")
        ctx.done
    }
      .onEvent {
        ...
        case (LocationAdded(loc), state) =>
          state.copy(locations = state.locations :+ loc)
      }
}