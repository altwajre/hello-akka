...
object RetrieveTokenRequest {
  implicit val retrieveTokenRequestFormat:
    Format[RetrieveTokenRequest] =
    Json.format[RetrieveTokenRequest]
}

object RetrieveTokenResult {
  implicit val retrieveTokenResultFormat:
    Format[RetrieveTokenResult] = Json.format[RetrieveTokenResult]
}

object ValidateTokenRequest {
  implicit val validateTokenRequestFormat:
    Format[ValidateTokenRequest] =
    Json.format[ValidateTokenRequest]
}

object ValidateTokenResult {
  implicit val validateTokenResultFormat:
    Format[ValidateTokenResult] = Json.format[ValidateTokenResult]
}