package com.packt.chapter6.sec12
...
object FriendJournalReader extends App {
  ...
  queries.eventsByPersistenceId("Laura").map(e => logEvent(e.persistenceId, e.event)).to(Sink.ignore).run()
  queries.eventsByPersistenceId("Maria").map(e => logEvent(e.persistenceId, e.event)).to(Sink.ignore).run()

  def logEvent(id: String, evt: Any) = system.log.info(s"Id [$id] Event [$evt]")
